...
@Component({
  selector: 'evendemy-meeting-new',
  templateUrl: './meeting-new.component.html',
  styleUrls: ['./meeting-new.component.scss']
})
export class MeetingNewComponent extends BaseComponent implements OnInit, OnDestroy {
  isNew: boolean;
  meeting: Meeting;
  ...
  private initForCreation(isIdea) {
    ...
    this.steps = [
      {href: this.meeting.isIdea ? 'ideas' : 'meetings2', title: this.meeting.isIdea ? 'Ideas' : 'Meetings'},
      {title: 'new'}
    ];
  }
